...
/*
 * Fixed-address app metadata footer: the last 2 bytes of flash, holding the
 * app's FW_VERSION as big-endian bytes (see firmware/src/main.cpp,
 * FW_VERSION_FOOTER). Because APPEND runs the app section to FLASHEND
 * regardless of app size, this address never moves as the app grows --
 * unlike a footer placed relative to the app's own end, it needs no rebuild
 * bookkeeping. A host (e.g. the ESPHome component) packaging a raw app image
 * for the I2C bootloader can read the version straight from the last 2 bytes
 * of that image, with no risk of drifting from what the flashed app will
 * actually report via REG_FW_VERSION -- both come from the same FW_VERSION
 * macro at compile time. Placed via `-Wl,--section-start=.fw_meta=...` in
 * platformio.ini (env:fb_app_only); NOT consumed by the bootloader itself.
 * ------------------------------------------------------------------------- */
#define BL_APP_META_ADDR       (BL_FLASH_SIZE - 2U)                     /* 0x3FFE */
